...
new = ImageTk.PhotoImage(resized_logo)
...
if int(dt.strftime('%I')) >= 8 and int(dt.strftime('%I')) <= 5:
    ...
    resized_moon = moon_image.resize((100, 100)) 
    img = ImageTk.PhotoImage(resized_moon)
    panel = Label(root, image=img)
    panel.place(x=210, y=200)
else:
    sun_image = Image.open('sun.png')
    resized_sun = sun_image.resize((100, 100))  
    img = ImageTk.PhotoImage(resized_sun)
    panel = Label(root, image=img)
    panel.place(x=210, y=200)
